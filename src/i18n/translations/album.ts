export const album = {
  en: {
    "album.title": "Album | Behangmotief — Music & festival photographer",
    "album.description":
      "View all photos from an album. Go through my archive of over 6 years of music photography.",
    "album.descriptions": [
      "Explore an electrifying collection of live photos capturing {artist}'s unforgettable performance at {venue}. Taken by Behangmotief, a music photographer known for vibrant, high-energy shots.",
      "Experience the energy and excitement of {artist} live at {venue} through this exclusive photo collection by Behangmotief, capturing every thrilling moment of the performance.",
      "Get front and center to {artist}'s performance at {venue} with these dynamic photos by Behangmotief, offering a vivid glimpse into the event's unforgettable atmosphere.",
      "Step into the live music experience with Behangmotief's photography collection of {artist} at {venue}. Each photo captures the intensity and spirit of this memorable performance.",
      "Relive the excitement of {venue} through Behangmotief's lens with a stunning array of photos from {artist}'s live set. High-energy moments and crowd passion, all in one album.",
      "Behangmotief presents a vivid gallery of {artist} performing at {venue}. From the stage energy to the crowd vibes, these photos bring you into the heart of the action.",
      "See {artist} like never before with Behangmotief's photography at {venue}. This photo collection captures the electric atmosphere and unforgettable moments from the show.",
      "Dive into the vibrant atmosphere of {venue} with Behangmotief's striking photos of {artist}. These images showcase the energy and intensity of live music at its finest.",
      "Captured by Behangmotief, this photo album of {artist} at {venue} offers a unique view of the live experience, with every image highlighting the energy and passion of the crowd.",
      "Discover the magic of live music with Behangmotief's photo collection of {artist} at {venue}. Each shot reflects the essence of music performance and the thrill of the audience.",
    ],
    "album.back": "Back",
    "album.more_from_artist": "Other performances by {artist}",
    "album.more_at_venue": "Same venue: {venue}",
  },
  nl: {
    "album.title": "Album | Behangmotief — Muziek & festivalfotograaf",
    "album.description":
      "Bekijk alle foto's uit een album. Doorloop mijn archief van meer dan 6 jaar muziekfotografie.",
    "album.descriptions": [
      "Ontdek een opwindende collectie live foto's die {artist}'s onvergetelijke optreden bij {venue} vastleggen. Genomen door Behangmotief, een muziekfotograaf bekend om levendige, energieke foto's.",
      "Ervaar de energie en opwinding van {artist} live bij {venue} via deze exclusieve fotocollectie van Behangmotief, die elk spannend moment van het optreden vastlegt.",
      "Kom vooraan bij {artist}'s optreden bij {venue} met deze dynamische foto's van Behangmotief, die een levendige blik bieden op de onvergetelijke sfeer van het evenement.",
      "Stap in de live muziekervaring met Behangmotief's fotocollectie van {artist} bij {venue}. Elke foto legt de intensiteit en spirit van dit memorabele optreden vast.",
      "Herbeleef de opwinding van {venue} door Behangmotief's lens met een prachtige reeks foto's van {artist}'s live set. High-energy momenten en publiekspassie, alles in één album.",
      "Behangmotief presenteert een levendige galerij van {artist} optredend bij {venue}. Van de podiumenergie tot de crowd vibes, deze foto's brengen je naar het hart van de actie.",
      "Zie {artist} als nooit tevoren met Behangmotief's fotografie bij {venue}. Deze fotocollectie legt de elektrische sfeer en onvergetelijke momenten van de show vast.",
      "Duik in de levendige sfeer van {venue} met Behangmotief's opvallende foto's van {artist}. Deze beelden tonen de energie en intensiteit van live muziek op zijn best.",
      "Vastgelegd door Behangmotief, dit fotoalbum van {artist} bij {venue} biedt een uniek beeld van de live ervaring, waarbij elke afbeelding de energie en passie van het publiek belicht.",
      "Ontdek de magie van live muziek met Behangmotief's fotocollectie van {artist} bij {venue}. Elke foto weerspiegelt de essentie van muziekperformance en de spanning van het publiek.",
    ],
    "album.back": "Terug",
    "album.more_from_artist": "Andere optredens van {artist}",
    "album.more_at_venue": "Zelfde locatie: {venue}",
  },
};
